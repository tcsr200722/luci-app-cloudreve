...
local api_url = "https://api.github.com/repos/HFO4/Cloudreve/releases/latest"
local download_url = "https://github.com/HFO4/Cloudreve/archive/"
...
function get_project_directory()
	return uci:get("cloudreve", "global", "project_directory") or luci.sys.exec("echo -n `uci get cloudreve.@global[0].project_directory`")
end
...
function to_download(url)
	...
	sys.call("/bin/rm -f /tmp/cloudreve_download.*")

	local tmp_file = util.trim(util.exec("mktemp -u -t cloudreve_download.XXXXXX"))
...
function to_extract(file)
	...
	sys.call("/bin/rm -rf /tmp/cloudreve_extract.*")
	local tmp_dir = util.trim(util.exec("mktemp -d -t cloudreve_extract.XXXXXX"))
...
function to_move(file)
	...
	sys.call("cp -R "..file.."/cloudreve*/* "..client_file)
	sys.call("/bin/rm -rf /tmp/cloudreve_extract.*")
	sys.call("chmod 777 "..file.."/cloudreve*/* "..client_file)
